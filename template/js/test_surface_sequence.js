...
(function($) {
    ...
    class NamedSurface {
        ...
        add_surfaces(scene) {
            // add surfaces as separate meshes
            var s = this.surfaces;
            for (var name in s) {
                var surface = s[name];
                surface.add_mesh(scene);
            }
        };
        remove_surfaces(scene) {
            // remove surfaces as separate meshes
            var s = this.surfaces;
            for (var name in s) {
                var surface = s[name];
                surface.remove_mesh(scene);
            }
        };
        set_arrays() {
            // combine arrays from surface components, for a single mesh...
            // This method seems to result in depth sorting anomalies in some cases(?)
            var npos = 0;
            var nindices = 0;
